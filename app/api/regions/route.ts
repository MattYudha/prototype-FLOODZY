...
export async function GET(request: Request) {
  ...
  console.log(
    `DEBUG: SUPABASE_URL available: ${!!process.env.NEXT_PUBLIC_SUPABASE_URL}`,
  );
  console.log(
    `DEBUG: SUPABASE_SERVICE_ROLE_KEY available: ${!!process.env.SUPABASE_SERVICE_ROLE_KEY}`,
  );
  ...
  try {
    ...
    console.log(
      `API Regions: Extracted type='${type}' and parentCode='${parentCode}'`,
    );
    ...
    console.log(
      `API Regions: Building query for table '${tableName}' with type '${type}'.`,
    );
    ...
    try {
      ...
      if (connectionTest.error) {
        console.error(
          'API Regions: Supabase connection test failed:',
    ...
    if (sortColumn) {
      ...
      console.warn(
        'API Regions: No clear column for ordering found in selectColumns.',
      );
    }
    ...
    if (error) {
      console.error(
        'API Regions: Error fetching data from Supabase:',
        error.message,
      );
      return NextResponse.json(
        { error: error.message },
    ...
    console.log(
      `API Regions: Data fetched successfully. Count: ${data?.length}`,
    );
    return NextResponse.json(data, { headers });
  } catch (error: any) {
    console.error(
      'API Regions: Unexpected error in GET handler:',
      error.message,
    );
    return NextResponse.json(
