...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!historicalData || !userPrompt) {
      console.warn("[Gemini Analysis API] ⚠️ 'historicalData' or 'userPrompt' missing in request.");
      return NextResponse.json(
        { error: "'historicalData' and 'userPrompt' are required in request body." },
        { status: 400 }
      );
    }
...
`;
    ...
    console.log(`[Gemini Analysis API] ✉️ Sending prompt to Gemini. Prompt length: ${prompt.length}`);
    ...
    console.error("[Gemini Analysis API] ❌ Error generating analysis:", error?.message);
    if (error?.stack) {
